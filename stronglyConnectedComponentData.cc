...
void revDFS(vector<int> &scc, int node, unordered_map<int, list<int>> &transpose, unordered_map<int, bool> &visited)
{
    visited[node] = true;
    for (auto i : transpose[node])
    {
        if (!visited[i])
        {
            revDFS(scc, i, transpose, visited);
        }
    }
    scc.push_back(node);
}
void DFS(int vertex, vector<pair<int, int>> &edges)
{
    ...
    while (!st.empty())
    {
        ...
        {
            vector<int> scc;
            revDFS(scc, trnFront, transpose, visited);
